...
func (pos *Position) Update(m *Move) *Position {
	// Keep Position and Board in one allocation. The board pointer keeps the
	// enclosing positionUpdate alive when only the returned Position is held.
	result := &positionUpdate{}
	pos.UpdateInto(&result.position, &result.board, m)
	return &result.position
}

// UpdateInto writes the position resulting from m into caller-owned storage.
// It is intended for bounded search code that can keep both values on its
// goroutine stack. dst and board must not alias pos or pos.Board().
func (pos *Position) UpdateInto(dst *Position, board *Board, m *Move) {
...
func (pos *Position) samePosition(pos2 *Position) bool {
	// Board is entirely composed of comparable bitboards and square values. An
	// exact value comparison avoids materializing two FEN strings for every
	// historical position checked after every move.
