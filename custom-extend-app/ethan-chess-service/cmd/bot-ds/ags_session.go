...
// made a one-second bot build take more than ten minutes. These are the only
// two Session endpoints the DS needs before its signaling layer starts.
...
func (c *agsSessionClient) sessionByID(ctx context.Context, sessionID string) (*claimedSession, error) {
	token, err := c.token(ctx); if err != nil { return nil, err }
	path := fmt.Sprintf("/session/v1/public/namespaces/%s/gamesessions/%s", url.PathEscape(c.namespace), url.PathEscape(sessionID))
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, c.baseURL+path, nil); if err != nil { return nil, err }
	req.Header.Set("Authorization", "Bearer "+token)
	resp, err := c.httpClient.Do(req); if err != nil { return nil, err }; defer resp.Body.Close()
	if resp.StatusCode/100 != 2 { return nil, fmt.Errorf("AGS session read returned %s", resp.Status) }
	var session claimedSession
	if err := json.NewDecoder(io.LimitReader(resp.Body, 256<<10)).Decode(&session); err != nil { return nil, err }
	return &session, nil
}

func (c *agsSessionClient) patchStorage(ctx context.Context, sessionID string, storage map[string]json.RawMessage) error {
	token, err := c.token(ctx); if err != nil { return err }
	body, err := json.Marshal(map[string]any{"storage": storage}); if err != nil { return err }
	path := fmt.Sprintf("/session/v1/public/namespaces/%s/gamesessions/%s", url.PathEscape(c.namespace), url.PathEscape(sessionID))
	req, err := http.NewRequestWithContext(ctx, http.MethodPatch, c.baseURL+path, strings.NewReader(string(body))); if err != nil { return err }
	...
	if resp.StatusCode/100 != 2 { return fmt.Errorf("AGS session storage update returned %s", resp.Status) }
	return nil
}

type fionaSignal struct {
...
func (c *agsSessionClient) answerSignal(ctx context.Context, sessionID string, bot *botbrain.Bot) error {
	for {
		session, err := c.sessionByID(ctx, sessionID); if err != nil { return err }
		var signal fionaSignal
		if raw := session.Storage["fionaSignal"]; len(raw) != 0 { _ = json.Unmarshal(raw, &signal) }
		if signal.SessionID == sessionID && signal.Nonce != "" && signal.Offer != nil && signal.Answer == nil {
			answer, _, err := botgame.AnswerContext(ctx, *signal.Offer, bot.Style, bot.ID); if err != nil { return err }
			signal.Answer = &answer
			raw, _ := json.Marshal(signal)
			if session.Storage == nil { session.Storage = map[string]json.RawMessage{} }
			session.Storage["fionaSignal"] = raw
			return c.patchStorage(ctx, sessionID, session.Storage)
		}
		select { case <-ctx.Done(): return ctx.Err(); case <-time.After(750 * time.Millisecond): }
	}
}

func newAGSSessionClient() (*agsSessionClient, bool) {
	value := func(botKey, defaultKey string) string {
		if v := strings.TrimSpace(os.Getenv(botKey)); v != "" { return v }
...
func (c *agsSessionClient) token(ctx context.Context) (string, error) {
	...
	if resp.StatusCode/100 != 2 { return "", fmt.Errorf("AGS login returned %s", resp.Status) }
	...
	if result.AccessToken == "" { return "", fmt.Errorf("AGS login returned no access token") }
...
func (c *agsSessionClient) claimedSession(ctx context.Context, podName string) (*claimedSession, error) {
	...
	if resp.StatusCode/100 != 2 { return nil, fmt.Errorf("AGS session lookup returned %s", resp.Status) }
	var session claimedSession
	if err := json.NewDecoder(io.LimitReader(resp.Body, 256<<10)).Decode(&session); err != nil { return nil, err }
	if session.ID == "" { return nil, fmt.Errorf("AGS session lookup returned no session id") }
	return &session, nil
}
...
func (c *agsSessionClient) setReady(ctx context.Context, sessionID string) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, c.baseURL+path, strings.NewReader(`{"ready":true}`)); if err != nil { return err }
	req.Header.Set("Authorization", "Bearer "+token); req.Header.Set("Content-Type", "application/json")
	resp, err := c.httpClient.Do(req); if err != nil { return err }; defer resp.Body.Close()
	if resp.StatusCode/100 != 2 { return fmt.Errorf("AGS set-ready returned %s", resp.Status) }
	return nil
}
...
func (c *agsSessionClient) waitForClaim(ctx context.Context, podName string) (*claimedSession, error) {
	for {
		...
		if err == nil && session != nil { return session, nil }
